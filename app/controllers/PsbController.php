...
class PsbController extends Controller
{
    ...
    public function mulaiPendaftaran()
    {
        ...
        // Result berupa array ['id' => ..., 'no_pendaftaran' => ...]
        if ($result && isset($result['id'])) {
            $idPendaftar = $result['id'];
            ...
            try {
                ...
                $namaSiswa = $pendaftar['nama_lengkap'] ?? $akun['nama_lengkap'] ?? 'Calon Siswa';
                $noPendaftaran = $pendaftar['no_pendaftaran'] ?? $result['no_pendaftaran'] ?? '-';
                $namaLembaga = $periode['nama_lembaga'] ?? '-';

                $message = "📝 *Pendaftaran Dimulai*\n\n";
                $message .= "Halo *" . $namaSiswa . "*,\n\n";
                $message .= "Pendaftaran Anda telah dibuat dengan:\n";
                $message .= "No. Pendaftaran: *" . $noPendaftaran . "*\n";
                $message .= "Lembaga: " . $namaLembaga . "\n\n";
    ...
    private function getFormDataBySection($section)
    {
        ...
        // Date fields that should be NULL if empty (not sent if empty)
        $dateFields = [
            'tanggal_lahir',
            'ayah_tanggal_lahir',
            'ibu_tanggal_lahir',
        ...
        foreach ($fields as $field) {
            if (isset($_POST[$field])) {
                ...
                // Skip empty date fields (don't add to data array)
    ...
    public function uploadDokumen($idPendaftar = null)
    {
        ...
        try {
            ...
            $dbUploadDir = 'public/uploads/psb/' . $idPendaftar . '/';
            $uploadDir = APPROOT . '/' . $dbUploadDir;
            if (!is_dir($uploadDir)) {
                mkdir($uploadDir, 0755, true);
            }
            ...
            // 1. Cek Koneksi Google Drive (STRICT MATCH)
            if (!file_exists(APPROOT . '/app/core/GoogleDrive.php')) {
                echo json_encode(['success' => false, 'message' => 'Modul Google Drive tidak terinstall']);
                return;
            }
            ...
            try {
                ...
                if (!$drive->isConnected()) {
                    echo json_encode(['success' => false, 'message' => 'Google Drive belum tersambung. Hubungi Admin Sekolah.']);
                    return;
                }
                $useGoogleDrive = true;
            } catch (Exception $e) {
                echo json_encode(['success' => false, 'message' => 'Gagal inisialisasi Google Drive: ' . $e->getMessage()]);
                return;
            }
            ...
            if ($useGoogleDrive) {
                ...
                try {
                    ...
                    $pendaftar = $this->psbModel->getPendaftarById($idPendaftar);
                    // Sanitasi dan format nama folder: NISN_NamaLengkap (spasi -> underscore)
                    $namaFolder = $pendaftar['nisn'] ?? $pendaftar['no_pendaftaran'];
                    $namaFolder = preg_replace('/[^A-Za-z0-9_\-]/', '_', $namaFolder);

                    $namaSiswaClean = preg_replace('/[^A-Za-z0-9_\-\s]/', '', $pendaftar['nama_lengkap']);
                    $namaFolder = $namaFolder . '_' . str_replace(' ', '_', $namaSiswaClean);
                    ...
                    if ($uploadResult && isset($uploadResult['id'])) {
                        ...
                        // Gagal upload ke drive, return error
                        $errMsg = isset($uploadResult['error']) ? $uploadResult['error'] : 'Unknown drive error';
                        error_log("GoogleDrive upload failed: " . $errMsg);
                        echo json_encode(['success' => false, 'message' => 'Gagal upload: ' . $errMsg]);
                        return;
                    }
                } catch (Exception $e) {
                    error_log("GoogleDrive upload error: " . $e->getMessage());
                    echo json_encode(['success' => false, 'message' => 'Error Google Drive: ' . $e->getMessage()]);
                    return;
                }
            }

            // Fallback lokal dinonaktifkan
            error_log("PSB Upload: Local fallback blocked");
            echo json_encode(['success' => false, 'message' => 'Gagal dokumen ke Google Drive. Hubungi Admin Sekolah.']);
            return;
...
<body>
    ...
    public function simpanPengaturan()
    {
        ...
        $popupGambar = $currentSettings['popup_gambar'] ?? null;
        if (isset($_FILES['popup_gambar']) && $_FILES['popup_gambar']['error'] === UPLOAD_ERR_OK) {
            $uploadDir = APPROOT . '/public/uploads/psb/popup/';
            if (!is_dir($uploadDir)) {
                mkdir($uploadDir, 0755, true);
            }

            $allowedTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
            $fileType = $_FILES['popup_gambar']['type'];

            if (in_array($fileType, $allowedTypes)) {
                $ext = pathinfo($_FILES['popup_gambar']['name'], PATHINFO_EXTENSION);
                $newName = 'popup_' . time() . '.' . $ext;
                $targetPath = $uploadDir . $newName;

                if (move_uploaded_file($_FILES['popup_gambar']['tmp_name'], $targetPath)) {
                    // Delete old popup if exists
                    if ($popupGambar && file_exists($uploadDir . $popupGambar)) {
                        unlink($uploadDir . $popupGambar);
                    }
                    $popupGambar = $newName;
                }
            }
        ...
        if (isset($_POST['hapus_popup']) && $_POST['hapus_popup'] == '1') {
            $uploadDir = APPROOT . '/public/uploads/psb/popup/';
            if ($popupGambar && file_exists($uploadDir . $popupGambar)) {
                unlink($uploadDir . $popupGambar);
            }
        ...
        $data = [
            ...
            // Popup settings
            'popup_aktif' => isset($_POST['popup_aktif']) ? 1 : 0,
            'popup_gambar' => $popupGambar,
            'popup_judul' => trim($_POST['popup_judul'] ?? ''),
            'popup_link' => trim($_POST['popup_link'] ?? ''),
            'popup_frequency' => trim($_POST['popup_frequency'] ?? 'once_session')
        ];
