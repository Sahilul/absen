...
class WaliKelasController extends Controller
{
    ...
    public function dashboard()
    {
        if ($_SESSION['role'] !== 'wali_kelas') {
            // Prevent infinite loop for Guru role
...
<body>
    ...
    public function prosesPembayaran()
    {
        ...
        try {
            // $lastId returned instead of boolean
            $lastId = $this->model('Pembayaran_model')->createTransaksi($tagihan_id, $id_siswa, $jumlah, $metode, $keterangan, $bukti_path, $_SESSION['user_id'] ?? null);
            if ($lastId) {
                // === Kirim Notifikasi ===
                error_log("[DEBUG] calling sendPembayaranNotification for ID: " . $lastId);
                $this->sendPembayaranNotification($lastId);

                Flasher::setFlash('Pembayaran berhasil dicatat dan notifikasi dikirim.', 'success');
                header('Location: ' . BASEURL . '/waliKelas/pembayaranTagihan/' . $tagihan_id);
            } else {
    ...
    {
        try {
            error_log("[DEBUG] sendPembayaranNotification START - Transaction ID: " . $id_transaksi);
            $trx = $this->model('Pembayaran_model')->getTransaksiById($id_transaksi);
            if (!$trx) {
                error_log("[DEBUG] Transaction NOT FOUND for ID: " . $id_transaksi);
                return;
            }

            $siswa = $this->model('Siswa_model')->getSiswaById($trx['id_siswa']);
            if (!$siswa) {
                error_log("[DEBUG] Siswa NOT FOUND for ID: " . $trx['id_siswa']);
                return;
            }
            ...
            $cleanNumber = function ($num) {
                ...
                // Log cleaned number for debugging
                return (strlen($n) > 7) ? $n : '';
            };
            ...
            error_log("[DEBUG] Phone Numbers - Ayah: '$ayahNo', Ibu: '$ibuNo', Wali: '$waliNo'");
            error_log("[DEBUG] Raw Phone Data - Ayah: '" . ($siswa['ayah_no_hp'] ?? '') . "'");
            ...
            error_log("[DEBUG] Targets found: " . count($targets) . " recipients");
            foreach ($targets as $t) {
                error_log("[DEBUG] Sending to: " . $t['nama'] . " (" . $t['no'] . ")");
                ...
                error_log("[DEBUG] Send Result for " . $t['no'] . ": " . print_r($result, true));
            }
        } catch (Throwable $e) {
    ...
    public function hapusTransaksi()
    {
        ...
        try {
            ...
            // Verify wali kelas owns this class (skip for bendahara/admin)
            $id_guru = $_SESSION['id_ref'] ?? 0;
            $id_tp_aktif = $_SESSION['id_tp_aktif'] ?? 0;
            $waliKelasInfo = $this->model('WaliKelas_model')->getWaliKelasByGuru($id_guru, $id_tp_aktif);
            ...
            // Bendahara dan admin bisa hapus transaksi dari semua kelas
            if (!$isBendahara && $role !== 'admin') {
                if (!$tagihan || !$waliKelasInfo || $tagihan['id_kelas'] != $waliKelasInfo['id_kelas']) {
    ...
    public function bayarLunas()
    {
        ...
        try {
            ...
            $lastId = $this->model('Pembayaran_model')->createTransaksi($tagihan_id, $id_siswa, $sisa, $metode, $keterangan, null, $_SESSION['user_id'] ?? null);

            if ($lastId) {
                // === Kirim Notifikasi ===
                error_log("[DEBUG] calling sendPembayaranNotification (Bayar Lunas) for ID: " . $lastId);
                $this->sendPembayaranNotification($lastId);
    ...
    public function pembayaranTagihanPDF($tagihan_id)
    {
        ...
        if (!$isBendahara && $role !== 'admin' && $role !== 'bendahara') {
...
<body>
    ...
    public function invoiceThermalData($tagihan_id, $id_siswa)
    {
        ...
        if ($role !== 'wali_kelas' && $role !== 'admin' && !$isBendahara && $role !== 'bendahara') {
            header('Content-Type: application/json');
