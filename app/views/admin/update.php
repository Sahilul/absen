...
<script>
    ...
    async function checkForUpdates() {
        ...
        try {
            ...
            if (d.has_update) {
                s.innerHTML = '<div class="text-green-600 font-medium">✨ Update tersedia: v' + d.latest + '</div><div class="text-sm text-gray-500">Saat ini: v' + d.current + '</div>';
                b.disabled = false;
                if (d.changelog) { document.getElementById('changelogContent').innerHTML = d.changelog.replace(/\n/g, '<br>'); document.getElementById('changelogSection').classList.remove('hidden'); }
            } else {
                s.innerHTML = '<div class="text-blue-600 font-medium">✓ Sudah terbaru (v' + d.current + ')</div>';
