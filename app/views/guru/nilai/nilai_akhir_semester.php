      ...
      <div class="bg-gradient-to-r from-success-500 to-emerald-600 p-6">
        <div class="flex items-center justify-between text-white">
          ...
          <div class="flex items-center gap-3">
            <a href="<?= BASEURL; ?>/nilai/downloadNilaiSASPDF?id_penugasan=<?= $data['penugasan']['id_penugasan']; ?>" 
...
let excelInputListener = false; // Flag untuk track apakah listener sudah terpasang
...
function initExcelListener() {
  ...
  reader.onload = function(e) {
    try {
      ...
      // Ambil seluruh sheet sebagai array-of-arrays
      ...
      // Berdasarkan template CBT: header kolom (NO, NOMOR UJIAN, NAMA, ..., HASIL AKHIR, ...) ada di baris ke-3
      const headerRowIndex = 2; // index 2 = baris ke-3
      const headerRow = sheetArray[headerRowIndex] || [];

      // Helper normalisasi teks header (case-insensitive, trim)
      const normalizeHeader = (h) => String(h || '').toUpperCase().trim();

      // Cari index kolom NOMOR UJIAN dan HASIL AKHIR (case-insensitive)
      ...
      const idxHasilAkhir = headerRow.findIndex(h => {
        ...
        // Terima beberapa variasi penulisan kolom nilai akhir
      ...
      // Data siswa dimulai setelah baris header
      const dataRows = sheetArray.slice(headerRowIndex + 1);

      // Simpan data + index kolom ke variabel global
      excelDataGlobal = {
        rows: dataRows,
        idxNomorUjian,
        idxHasilAkhir
...
function openImportModal() {
  ...
  initExcelListener(); // Pasang listener saat modal dibuka
}
...
function processImport() {
  ...
  // Proses data dengan sedikit delay agar UI ter-update
  setTimeout(() => {
...
function processExcelData(data, idxNomorUjian, idxHasilAkhir) {
  ...
  data.forEach(row => {
    ...
    // Cari input field berdasarkan NISN
    const inputs = document.querySelectorAll('input[name^="nilai["]');
    let found = false;
    ...
    inputs.forEach(input => {
      ...
      const nisnCell = row.querySelector('td:nth-child(2)'); // NISN di kolom ke-2
      if (nisnCell && nisnCell.textContent.trim() === nomorUjian) {
        // Parse nilai (hapus koma, ambil angka saja)
        const nilai = parseFloat(String(nilaiAkhir).replace(',', '.'));
        if (!isNaN(nilai) && nilai >= 0 && nilai <= 100) {
          input.value = Math.round(nilai); // Bulatkan ke integer
          imported++;
          found = true;
        }
      }
  ...
  // Auto-submit form setelah import selesai
  if (imported > 0) {
  ...
  <div class="glass-effect rounded-2xl border border-white/20 shadow-2xl max-w-2xl w-full animate-scale-up">
    ...
    <div class="p-6">
      <form id="importForm" class="space-y-6">
        <!-- Info Box -->
        <div class="glass-effect rounded-xl p-4 border border-primary-200 bg-gradient-to-br from-primary-50 to-blue-50">
          <div class="flex items-start gap-3">
            <div class="bg-primary-100 p-2 rounded-lg">
              <i data-lucide="info" class="w-5 h-5 text-primary-600"></i>
            </div>
            <div class="flex-1">
              <h4 class="font-bold text-primary-900 mb-2">Petunjuk Import:</h4>
              <ul class="text-sm text-primary-800 space-y-1 list-disc list-inside">
                ...
                <li>Kolom <strong>NILAI AKHIR</strong> akan diimport sebagai nilai SAS</li>
        ...
        <div>
          ...
          <input type="file" id="excelFile" accept=".xlsx,.xls" required
                 class="block w-full text-sm text-secondary-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 transition-colors cursor-pointer border border-secondary-300 rounded-lg">
    ...
    <div class="p-6 bg-secondary-50 rounded-b-2xl border-t border-secondary-200 flex justify-end gap-3">
      ...
      <button type="button" id="btnProcessImport" onclick="processImport()" class="btn-primary py-2 px-6" disabled>
...
<script>
  // Wait for Lucide library to load then create icons
