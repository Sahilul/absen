...
<aside id="sidebar" class="sidebar fixed top-0 left-0 z-40 w-64 h-screen flex flex-col border-r border-white/20 shadow-2xl
  ...
  <div
    ...
    <div class="flex items-center">
      ...
      <div class="ml-3 min-w-0">
        ...
        <p class="text-xs text-secondary-500 font-medium mt-0.5">Kepala Madrasah</p>
      </div>
    </div>
    ...
    <ul class="space-y-2">
      ...
      <li>
        <a href="<?= BASEURL; ?>/KepalaMadrasah/dashboard"
          ...
          <div
            class="<?= ($judul == 'Dashboard Monitoring') ? 'bg-white/20' : 'bg-secondary-100 group-hover:bg-primary-100'; ?> p-2 rounded-lg transition-colors duration-200">
            <i data-lucide="layout-dashboard"
              class="w-4 h-4 <?= ($judul == 'Dashboard Monitoring') ? 'text-white' : 'text-secondary-500 group-hover:text-primary-600'; ?>"></i>
          </div>
      ...
      <li>
        <a href="<?= BASEURL; ?>/PerformaSiswa"
          ...
          <div
            class="<?= ($judul == 'Performa Kehadiran Siswa') ? 'bg-white/20' : 'bg-secondary-100 group-hover:bg-purple-100'; ?> p-2 rounded-lg transition-colors duration-200">
            <i data-lucide="user-check"
              class="w-4 h-4 <?= ($judul == 'Performa Kehadiran Siswa') ? 'text-white' : 'text-secondary-500 group-hover:text-purple-600'; ?>"></i>
          </div>
      ...
      <li>
        <a href="<?= BASEURL; ?>/PerformaGuru"
          ...
          <div
            class="<?= ($judul == 'Performa Kinerja Guru') ? 'bg-white/20' : 'bg-secondary-100 group-hover:bg-green-100'; ?> p-2 rounded-lg transition-colors duration-200">
            <i data-lucide="user-check-2"
              class="w-4 h-4 <?= ($judul == 'Performa Kinerja Guru') ? 'text-white' : 'text-secondary-500 group-hover:text-green-600'; ?>"></i>
          </div>
          ...
          <?php if ($judul != 'Performa Kinerja Guru'): ?>
            <div
              class="ml-auto bg-green-100 text-green-600 px-2 py-0.5 rounded-full text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              NEW
            </div>
          <?php endif; ?>
        </a>
  ...
  <div class="mx-4 mb-4">
    ...
    $namaApp = $namaAplikasi ?? 'Smart Absensi'; // Fallback variable
    ?>
    <div class="text-center text-xs text-secondary-400 mb-2">
      <span class="font-medium"><?= $namaApp; ?></span> • <span>v<?= $appVersion; ?></span>
    </div>
    <div class="p-4 bg-gradient-to-r from-primary-50 to-success-50 rounded-xl border border-white/20">
