        ...
        <?php foreach ($dokumenConfig as $doc):
            ...
            if ($isUploaded) {
                ...
                if ($isDrive) {
                    ...
                    $pathFile = $uploaded['path_file'];

                    // Check for absolute URL
                    if (strpos($pathFile, 'http') === 0) {
                        $previewUrl = $pathFile;
                    }
                    // Check for relative public path
                    else if (strpos($pathFile, 'public/') === 0 || strpos($pathFile, 'uploads/') === 0) {
                        $previewUrl = BASEURL . '/' . $pathFile;
                    } else {
                        // Legacy path fallback
                        $previewUrl = BASEURL . '/uploads/dokumen_siswa/' . $pathFile;
                    }
