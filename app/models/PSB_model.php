...
class PSB_model
{
    ...
    public function updatePengaturan($data)
    {
        $this->db->query('UPDATE psb_pengaturan SET 
                          ...
                          popup_aktif = :popup_aktif, popup_gambar = :popup_gambar,
                          popup_judul = :popup_judul, popup_link = :popup_link,
        ...
        $this->db->bind(':popup_aktif', $data['popup_aktif'] ?? 0);
        $this->db->bind(':popup_gambar', $data['popup_gambar'] ?? null);
        $this->db->bind(':popup_judul', $data['popup_judul'] ?? null);
        $this->db->bind(':popup_link', $data['popup_link'] ?? null);
        $this->db->bind(':popup_frequency', $data['popup_frequency'] ?? 'once_session');
        return $this->db->execute();
    }
