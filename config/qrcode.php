...
if (!defined('QR_WEBSITE_URL')) {
    ...
    // Coba ambil dari database jika helper tersedia
    if (function_exists('getPengaturanAplikasi')) {
        $appSettings = getPengaturanAplikasi();
        if (!empty($appSettings['url_web'])) {
            $qrBaseUrl = $appSettings['url_web'];
        }
    }

    // Pastikan ada trailing slash
    define('QR_WEBSITE_URL', rtrim($qrBaseUrl, '/') . '/');
}
