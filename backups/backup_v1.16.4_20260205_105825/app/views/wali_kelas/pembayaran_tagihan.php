...
<div class="p-4 sm:p-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-2xl font-bold text-gray-800"><?= $isBendahara ? 'Bendahara - ' : '' ?>Detail Tagihan</h1>
    ...
    <div class="flex flex-wrap gap-2">
      <?php if (!empty($data['tagihan']['id'])): ?>
        <a href="<?= $urlPrefix ?>/pembayaranTagihanPDF/<?= (int) $data['tagihan']['id'] ?>"
      ...
      <?php endif; ?>
      <a href="<?= $isBendahara ? BASEURL . '/bendahara/kelolaPembayaran/' . $idKelas : BASEURL . '/waliKelas/pembayaran' ?>"
  ...
  <?php if (empty($data['tagihan'])): ?>
    ...
    <div class="bg-gradient-to-br from-white to-blue-50 rounded-xl shadow-lg p-6 border border-blue-100">
      ...
      <form action="<?= $urlPrefix ?>/simpanTagihanKelas" method="POST" class="grid sm:grid-cols-5 gap-4">
    ...
    <div class="grid sm:grid-cols-2 gap-4 mb-6">
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-5 text-white">
        ...
        <div class="text-2xl font-bold">Rp
          <?= number_format((int) ($data['tagihan']['nominal_default'] ?? 0), 0, ',', '.') ?>
        </div>
      </div>
          ...
          <div
            ...
            <div class="flex flex-wrap gap-2">
              <button type="button"
                ...
                onclick="openInputModal(<?= (int) ($data['tagihan']['id'] ?? 0) ?>, <?= (int) $s['id_siswa'] ?>, <?= htmlspecialchars(json_encode($s['nama_siswa'])) ?>, <?= max(0, (int) $nominal - (int) $diskon - (int) $terbayar) ?>, <?= (int) $nominal ?>, <?= (int) $terbayar ?>)">
              ...
              if ($sisaRow > 0): ?>
                <button type="button"
                  ...
                  onclick="bayarLunas(this, <?= (int) ($data['tagihan']['id'] ?? 0) ?>, <?= (int) $s['id_siswa'] ?>, <?= htmlspecialchars(json_encode($s['nama_siswa'])) ?>, <?= $sisaRow ?>)">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
              ...
              <?php if ($status === 'lunas' || $status === 'sebagian'): ?>
                <button
                  onclick="lihatRiwayat(<?= (int) ($data['tagihan']['id'] ?? 0) ?>, <?= (int) $s['id_siswa'] ?>, <?= htmlspecialchars(json_encode($s['nama_siswa'])) ?>)"
                ...
                <a class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-lg text-sm font-medium shadow-sm transition-all"
                  href="<?= $urlPrefix ?>/invoicePembayaran/<?= (int) ($data['tagihan']['id'] ?? 0) ?>/<?= (int) $s['id_siswa'] ?>">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                    <polyline points="14 2 14 8 20 8" />
                ...
                <button
                  onclick="showPrintOptions(<?= (int) ($data['tagihan']['id'] ?? 0) ?>, <?= (int) $s['id_siswa'] ?>, <?= htmlspecialchars(json_encode($s['nama_siswa'])) ?>)"
...
<div id="modalInput" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
  <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
    ...
    <form id="formInputPembayaran" action="<?= $urlPrefix ?>/prosesPembayaran<?= $isBendahara ? 'Tagihan' : '' ?>"
...
<script>
  ...
  async function fetchThermalData() {
    ...
    try {
      const response = await fetch(`${URL_PREFIX}/invoiceThermalData/${currentPrintTagihanId}/${currentPrintSiswaId}`);
  ...
  async function printViaBluetooth() {
    ...
    try {
      ...
      const response = await fetch(`${URL_PREFIX}/invoiceThermalData/${tagihanId}/${siswaId}`);
      const result = await response.json();
...
<br><br><br><br><br>
  ...
  async function lihatRiwayat(tagihanId, siswaId, namaSiswa) {
    ...
    try {
      // Fetch transactions
      const response = await fetch(`${URL_PREFIX}/getRiwayatTransaksi/${tagihanId}/${siswaId}`);
      const result = await response.json();
      ...
      if (data.transaksi.length === 0) {
        ...
        const safeNamaSiswa = namaSiswa.replace(/'/g, "\\'");
        ...
        <div class="bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow">
          <div class="flex items-start justify-between mb-3">
            ...
            <button onclick="hapusTransaksi(${trx.id}, ${tagihanId}, ${siswaId}, '${safeNamaSiswa}')" class="ml-2 p-2 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-lg shadow-sm hover:shadow-md transition-all" title="Hapus transaksi">
  ...
  async function hapusTransaksi(transaksiId, tagihanId, siswaId, namaSiswa) {
    ...
    try {
      ...
      const response = await fetch(`${URL_PREFIX}/hapusTransaksi`, {
        method: 'POST',
        body: formData
      });
  ...
  // Bayar Lunas (AJAX)
  async function bayarLunas(btnElement, tagihanId, siswaId, namaSiswa, sisa) {
    // Disable button immediately using passed reference
    ...
    try {
      ...
      const res = await fetch(`${URL_PREFIX}/bayarLunas`, { method: 'POST', body: formData });
