<?php
// File: app/views/admin/psb/edit_periode.php
$p = $data['periode'];
?>
...
<div class="animate-fade-in">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-secondary-800">Edit Periode PSB</h1>
            <p class="text-secondary-500 mt-1"><?= htmlspecialchars($p['nama_periode']); ?></p>
        </div>
    </div>

    <form action="<?= BASEURL; ?>/psb/prosesUpdatePeriode" method="POST" class="space-y-6 max-w-3xl">
        <input type="hidden" name="id_periode" value="<?= $p['id_periode']; ?>">

        <div class="bg-white rounded-xl shadow-sm border p-6">
            <h2 class="text-lg font-semibold text-secondary-800 mb-4">Informasi Periode</h2>
                ...
                <div>
                    ...
                    <select name="id_lembaga" required
                        ...
                        <?php foreach ($data['lembaga'] as $l): ?>
                            <option value="<?= $l['id_lembaga']; ?>" <?= $l['id_lembaga'] == $p['id_lembaga'] ? 'selected' : ''; ?>><?= htmlspecialchars($l['nama_lembaga']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select name="id_tp" required
                        ...
                        <?php foreach ($data['tahun_pelajaran'] as $tp): ?>
                            <option value="<?= $tp['id_tp']; ?>" <?= $tp['id_tp'] == $p['id_tp'] ? 'selected' : ''; ?>>
                                <?= htmlspecialchars($tp['nama_tp']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="mt-5">
                <label class="block text-sm font-semibold text-secondary-700 mb-2">Nama Periode <span
                        class="text-danger-500">*</span></label>
                <input type="text" name="nama_periode" required value="<?= htmlspecialchars($p['nama_periode']); ?>"
                    class="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
                <div>
                    <label class="block text-sm font-semibold text-secondary-700 mb-2">Tanggal Buka <span
                            class="text-danger-500">*</span></label>
                    <input type="date" name="tanggal_buka" required value="<?= $p['tanggal_buka']; ?>"
                        class="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-secondary-700 mb-2">Tanggal Tutup <span
                            class="text-danger-500">*</span></label>
                    <input type="date" name="tanggal_tutup" required value="<?= $p['tanggal_tutup']; ?>"
                        class="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mt-5">
                <div>
                    ...
                    <input type="number" name="kuota" min="0" value="<?= $p['kuota']; ?>"
                        class="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-secondary-700 mb-2">Biaya Pendaftaran (Rp)</label>
                    <input type="number" name="biaya_pendaftaran" min="0" value="<?= $p['biaya_pendaftaran']; ?>"
                        class="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
            ...
            <div class="mt-5">
                ...
                <textarea name="keterangan" rows="2"
                    class="w-full px-4 py-3 border border-secondary-300 rounded-lg focus:ring-2 focus:ring-primary-500"><?= htmlspecialchars($p['keterangan'] ?? ''); ?></textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <?php foreach ($data['jalur'] as $j): 
                    $isAktif = ($j['kuota'] ?? 0) > 0;
                ?>
                    <div class="p-4 border rounded-lg transition-colors jalur-card <?= $isAktif ? 'border-primary-400 bg-primary-50' : 'border-secondary-200'; ?>" data-jalur="<?= $j['id_jalur']; ?>">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-secondary-400"><?= htmlspecialchars($j['kode_jalur'] ?? ''); ?> • Terisi: <?= $j['terisi'] ?? 0; ?></p>
                            </div>
                        </div>
                        <div class="mt-3 kuota-input <?= $isAktif ? '' : 'hidden'; ?>" id="kuota-<?= $j['id_jalur']; ?>">
                            <label class="block text-xs font-medium text-secondary-600 mb-1">Kuota</label>
                            <input type="number" name="kuota_jalur[<?= $j['id_jalur']; ?>]" min="0" value="<?= $j['kuota'] ?? 0; ?>"
