<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gradient-to-br from-secondary-50 to-secondary-100 p-4 md:p-6">
            ...
            <div class="flex flex-col gap-4">
                <div>
                    ...
                    <p class="text-sm md:text-base text-secondary-600">
                        Semester: <span
                            class="font-semibold text-primary-600"><?= $_SESSION['nama_semester_aktif']; ?></span>
                    </p>
                </div>
            </div>
        </div>
    ...
    <?php if (empty($data['tagihan_siswa'])): ?>
        ...
        <div class="glass-effect rounded-xl p-8 md:p-12 border border-white/20 shadow-lg text-center">
            <div
                class="w-16 h-16 md:w-24 md:h-24 bg-secondary-100 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
            ...
            <p class="text-sm md:text-base text-secondary-600 mb-4 md:mb-6">Belum ada tagihan pembayaran untuk semester ini.
            </p>
            <a href="<?= BASEURL; ?>/siswa/dashboard"
                class="btn-primary inline-flex items-center gap-2 text-sm md:text-base">
    ...
    <?php else: ?>
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="relative flex-1">
                        <i data-lucide="search"
                            class="absolute left-3 top-1/2 transform -translate-y-1/2 text-secondary-400 w-4 h-4"></i>
        ...
        <div class="block lg:hidden space-y-3 mb-6" id="mobileTagihanList">
            <?php foreach ($data['tagihan_siswa'] as $index => $tagihan): ?>
                ...
                // Gunakan update status dari database jika ada, atau hitung manual
                $status_db = $tagihan['status'] ?? 'belum';
                $is_lunas = ($status_db === 'lunas') || ($terbayar >= $harus_bayar && $harus_bayar > 0) || ($harus_bayar == 0);

                // Fix Progress: Jika lunas atau harus bayar 0 (full diskon), progress 100%
                if ($is_lunas || $harus_bayar == 0) {
                    $progress = 100;
                } else {
                ...
                <div class="glass-effect rounded-xl p-4 border border-white/20 shadow-lg animate-fade-in"
                    style="animation-delay: <?= $index * 50; ?>ms" data-row data-tagihan="<?= strtolower($tagihan['nama']); ?>"
                    ...
                    <div class="flex items-start justify-between mb-3">
                        <div class="flex items-start gap-3 flex-1">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span
                                    class="<?= $is_lunas ? 'status-lunas' : 'status-belum'; ?> px-2 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1">
                    ...
                    <div class="mt-3 pt-3 border-t border-secondary-200">
                        <button
                            onclick="showRiwayat(<?= $tagihan['id']; ?>, '<?= htmlspecialchars(addslashes($tagihan['nama'])); ?>')"
                    ...
                    <tbody class="bg-white divide-y divide-secondary-100" id="tableBody">
                        <?php foreach ($data['tagihan_siswa'] as $index => $tagihan): ?>
                            ...
                            <tr class="hover:bg-secondary-50 transition-colors duration-200 animate-fade-in"
                                style="animation-delay: <?= $index * 50; ?>ms" data-row
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div
                                            class="w-10 h-10 gradient-primary rounded-lg flex items-center justify-center mr-4">
                                            <i data-lucide="file-text" class="w-5 h-5 text-white"></i>
                                        </div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <span
                                            class="<?= $is_lunas ? 'status-lunas' : 'status-belum'; ?> px-3 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1">
                                            <i data-lucide="<?= $is_lunas ? 'check-circle' : 'alert-circle'; ?>"
                                                class="w-3 h-3"></i>
                                            <?= $is_lunas ? 'Lunas' : 'Belum Lunas'; ?>
                                        </span>
                                        <button
                                            ...
                                            <i data-lucide="history"
                                                class="w-4 h-4 text-primary-600 group-hover:text-primary-700"></i>
                                        </button>
                                    </div>
                                </td>
                            </tr>
