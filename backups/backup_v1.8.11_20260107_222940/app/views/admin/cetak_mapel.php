<?php
// File: app/views/admin/cetak_rincian_absen.php
...
// Ambil combo_id untuk navigation
$combo_id = '';
if (!empty($mapel_info['nama_guru']) && !empty($mapel_info['nama_mapel'])) {
    // Ekstrak combo_id dari URL atau build ulang dari data
    $url_path = $_SERVER['REQUEST_URI'] ?? '';
    if (preg_match('/cetakRincianAbsenAdmin\/([^?]+)/', $url_path, $matches)) {
        $combo_id = $matches[1];
    }
}
?>
...
<title>Rincian Absen Admin - <?= htmlspecialchars($mapel_info['nama_mapel'] ?? 'Mapel'); ?> (<?= htmlspecialchars($mapel_info['nama_guru'] ?? 'Guru'); ?>)</title>
<style>
  /* ===== Halaman & Global ===== */
...
<body>
    ...
    <div class="actions no-print">
        <div class="actions-buttons">
            ...
            <?php
            // Link PDF untuk admin
            $pdfParams = $_GET;
            $pdfParams['pdf'] = 1;
            $pdfParams['action'] = 'cetak';
            
            // Build PDF URL dengan combo_id
            $pdfUrl = BASEURL . '/admin/cetakRincianAbsenAdmin/' . htmlspecialchars($combo_id) . '?' . http_build_query($pdfParams);
            ?>
            <a class="btn" href="<?= htmlspecialchars($pdfUrl); ?>">
                <span class="btn-icon">📄</span>
                Unduh PDF
            </a>
            <a class="btn" href="<?= BASEURL; ?>/admin/rincianAbsenAdmin">
                <span class="btn-icon">←</span>
                Kembali
            </a>
        </div>
...
<div class="main-content">
  ...
  <div class="header-section">
    <div class="header-title">Rekapitulasi Absen Siswa (Admin)</div>
    ...
    <table class="info-table">
      ...
      <?php if (!empty($mapel_info['nama_tp'])): ?>
      <tr>
        <td class="info-label">Tahun Pelajaran</td><td class="info-colon">:</td>
        <td colspan="4"><?= htmlspecialchars($mapel_info['nama_tp']); ?></td>
      </tr>
      <?php endif; ?>
    </table>
  </div>
      ...
      <tr>
        <td style="width:60%;">
          <div class="keterangan">
            ...
            <div style="font-size:9px; font-style:italic; margin-top:2px; color:#666;">
              Laporan Admin - Data: <?= htmlspecialchars($mapel_info['nama_guru'] ?? 'Guru'); ?> | <?= htmlspecialchars($mapel_info['nama_mapel'] ?? 'Mapel'); ?>
            </div>
          </div>
        </td>
        <td style="width:40%;">
          <div class="signature">
            ...
            <div class="signature-role">Kepala Sekolah / Administrator</div>
            <div class="signature-name"><?= htmlspecialchars($_SESSION['nama_lengkap'] ?? '(Administrator)'); ?></div>
          </div>
        </td>
      </tr>
    </table>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    window.addEventListener('beforeprint', function () {
      const mapelName = '<?= preg_replace('/[^A-Za-z0-9]/', '_', $mapel_info['nama_mapel'] ?? 'Mapel'); ?>';
      const guruName = '<?= preg_replace('/[^A-Za-z0-9]/', '_', $mapel_info['nama_guru'] ?? 'Guru'); ?>';
      document.title = 'Rincian_Absen_Admin_' + mapelName + '_' + guruName + '_<?= date('Y-m-d'); ?>';
    });
  });
</script>
</body>
